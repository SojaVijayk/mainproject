  ...
  $user = Auth::user();
    ...
    $categorySummary = $categoryWise->map(function ($group) {
        return [
            ...
            'total_invoice_raised_tax' => $group->sum(fn($p) => $p->invoices->where('invoice_type',2)->sum('amount'))/100000,
            'total_invoice_raised_proforma' => $group->sum(fn($p) => $p->invoices->where('invoice_type',1)->sum('amount'))/100000,
            ...
            ->map(function ($catGroup) {
                return [
                    ...
                    'total_invoice_raised' => $catGroup->sum(fn($p) => $p->invoices->sum('amount')) / 100000,
                        'total_invoice_raised_tax' => $catGroup->sum(fn($p) => $p->invoices->where('invoice_type',2)->sum('amount'))/100000,
            'total_invoice_raised_proforma' => $catGroup->sum(fn($p) => $p->invoices->where('invoice_type',1)->sum('amount'))/100000,
